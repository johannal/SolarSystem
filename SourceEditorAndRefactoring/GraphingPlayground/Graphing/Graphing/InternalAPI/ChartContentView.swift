...
internal class ChartBackgroundView: UIView {
    ...
    func drawAxisLabelsAndGridlinesInContext(_ context: CGContext, chartView: ChartView) {
        ...
        for ((x, xScreenPos), xLabelImage) in zip(xValues, xLabelImages) where abs(x) > .ulpOfOne {
        ...
        for ((y, yScreenPos), yLabelImage) in zip(yValues, yLabelImages) where abs(y) > .ulpOfOne {
