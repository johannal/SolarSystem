...
internal class LinePlotDrawable: ChartDrawable {
    ...
    private func drawSymbolsAtPoints(points: [(Double, Double)], inChartView chartView: ChartView, inContext context: CGContext) {
        ...
        for point in points {
            ...
            var adjustedUse = color
            if shouldMakeDarker {
                adjustedUse = color.darker()
            }
            
            symbol.draw(atCenterPoint: CGPoint(x: screenPoint.x, y: screenPoint.y), fillColor: adjustedUse.uiColor, usingContext: context)
        }
    }
    
    // MARK: - ChartDrawable conformance -
    
    var color = Color.blue.darker(percent: 0.05)
    var label: String?
